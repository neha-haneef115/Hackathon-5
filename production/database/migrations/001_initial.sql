-- TechCorp FTE Database Migration 001 - Initial Schema
-- Wrapped in DO block for safe execution with IF NOT EXISTS
...
BEGIN
    ...
    CREATE INDEX IF NOT EXISTS idx_conversations_active_recent ON conversations(customer_id, started_at DESC) 
    ...
    CREATE INDEX IF NOT EXISTS idx_escalation_log_ticket_id ON escalation_log(ticket_id);
    CREATE INDEX IF NOT EXISTS idx_escalation_log_escalated_at ON escalation_log(escalated_at DESC);
    CREATE INDEX IF NOT EXISTS idx_escalation_log_urgency ON escalation_log(urgency);
    ...
    CREATE INDEX IF NOT EXISTS idx_agent_metrics_name_channel_time ON agent_metrics(metric_name, channel, recorded_at DESC);
    ...
    INSERT INTO agent_metrics (metric_name, metric_value, dimensions) VALUES
    ('migration_001_completed', 1, '{"version": "001", "timestamp": "' || NOW() || '"}')
